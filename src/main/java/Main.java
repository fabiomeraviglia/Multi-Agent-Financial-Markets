...
public class Main {
  ...
  public static void main(String[] args) {
    ...
    List<Agent> easyTacticAgents = sim.agents.stream()
            .filter(agent -> agent.getTactic() instanceof EasyTactic)
            .collect(Collectors.toList());

    Integer initialWorldValue = sim.agents.stream()
            .map(agent -> agent.getAssets().getCash())
            .reduce(0, (a, b) -> a + b);
    System.out.println("Cash totale nel mondo iniziale: " + initialWorldValue.toString());

    System.out.println("Asset iniziali agenti easy-tactic:");
